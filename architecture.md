...
| **v1.34.0** | **24.04.2026** | **Termin-Typ-Icons + Modal-Redesign**: (6) **Termin-Typ-Icons** — `terminTypIcon(wert)` mit Fuzzy-Match auf gängige deutsche Bezeichnungen (Call 📞, Meeting 🤝, Training/Schulung 🎓, Workshop 🛠️, Präsentation 📊, Akquise 🎯, Beratung 💬, Follow-up 🔁, Kickoff 🚀, Vor Ort 📍, Online 💻, Demo 🎬, Messe 🏢; Fallback 📅). Neuer **Icon-Picker** im Termin-Modal über dem Typ-Dropdown: klickbare Chips mit Icon + Label, Klick setzt Typ, synchronisiert mit Dropdown-Wert. Icons erscheinen zusätzlich **vor dem Titel** in allen Termin-Listen (Haupt, Firma/Kontakt/Projekt-Tab), im Kalender-Popover und im Termin-Inline-Dashboard (verwandte Termine derselben Firma). (9) **Modal-Redesign** — Sekundär-Sektionen standardmäßig kollabiert: im **Termin-Modal** „Notizen", im **Einsatz-Modal** „Techniker" und „Beschreibung & Notizen" (neue eigene Sektion, getrennt von Kopplung), im **Aufgabe-Modal** „Zuordnung (optional)" und „Notizen". Das verkürzt die sichtbare Höhe der Modals drastisch und bringt die Pflichtfelder direkt in den Fokus; Kollabierte Sektionen bleiben per Klick auf den Title aufklappbar (v1.9.x-Mechanismus). Einsatz bekommt zusätzlich eine eigene „Kopplung"-Sektion (Termin-Sync + Bonus-Einlösung) getrennt von den Notizen — fachliche Trennung von Metadaten und Content. Kein Schema-Change. |

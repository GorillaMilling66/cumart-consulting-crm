...
| **v1.33.0** | **24.04.2026** | **Fünferpack Usability**: (1) **Verwandte Aufgaben**: Kontakt-Name hat Vorrang vor Firma im Kunden-Label — präzisere Zuordnung. (2) **Plus-Button im Kalender-Popover**: öffnet Mini-Menü (+Termin/+Einsatz/+Aufgabe) mit Datum-Prefill auf den geklickten Tag — Direkt-Workflow „Tag sehen → Termin eintragen". (3) **Einsatz-Menge auto aus Werktagen**: Bei Änderung von `datum_von`/`datum_bis` wird `menge` automatisch auf die Anzahl Werktage im Bereich gesetzt (Mo–Fr, ohne BW-Feiertage). Manuelle Eingabe überschreibt; Reset-Icon ↻ neben `d-menge` rechnet bei Bedarf neu. Im Edit-Mode wird die existierende Menge nie überschrieben. (4) **Datum-Schnellauswahl** umgestellt: Heute · Nächster WT · +3 WT · +7 WT · Nächster Mo + drei dynamische Monats-Buttons (1. Werktag des jeweiligen Folgemonats). Generischer Helper `setDateShortcut(inputId, key)`, aktiv in Termin-, Einsatz- und Aufgabe-Modal — alle drei haben jetzt dieselben Buttons. (5) **Ganztags-Checkbox** in Termin- und Einsatz-Modal setzt Uhrzeit auf 08:00–16:00 und sperrt die Zeit-Inputs; beim Deaktivieren wieder freigegeben. Neue Hilfsfunktionen: `isWorkday`, `nextWorkdayAfter`, `addWorkdays`, `countWorkdaysInclusive`, `renderDateShortcuts`, `applyGanztag`, `recomputeDeploymentMengeFromDates`, `toggleCalendarQuickMenu`, `calendarQuickCreate{Appointment,Deployment,Task}`. Kein Schema-Change. |
